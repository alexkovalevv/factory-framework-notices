...
add_action('factory_notices_000_plugin_created', 'factory_notices_000_plugin_created');
function factory_notices_000_plugin_created( $plugin ) {
    new FactoryNotices000( $plugin );
}
...
class FactoryNotices000 {
    ...
    public function enqueueScripts() {
        wp_enqueue_style('factory-notices-000-css', FACTORY_NOTICES_000_URL . '/assets/css/notices.css');      
        wp_enqueue_script('factory-notices-000-js', FACTORY_NOTICES_000_URL . '/assets/js/notices.js');
    }
    ...
    public function showNotices() {
        ...
        <div class="updated factory-bootstrap-000 factory-fontawesome-000 factory-notices-000-notices">
        <?php
        foreach ($this->notices as $notice) {
    ...
    public function renderNoticeButton( $data, $id ) {
        ...
        $onclick = '';
        if ( $action == 'x' ) { 
            $onclick = "factory_notices_000_hide_notice('$id', false); return false;";
            $action = '#';
        }

        if ( $action == 'xx' ) { 
            $action = '#';
            $onclick = "factory_notices_000_hide_notice('$id', true); return false;"; 
        }
